...
jimport('cover.profilecover.profilecover');

/**
 * Mosets Profile Cover Plugin.
 *
 * @package	Mosets
 * @subpackage	Profile Cover
 * @version	1.0
 */
class plgUserProfileCover extends JPlugin
{
	/**
	 * @const  string
	 * @since  1.0
	...
	/**
	 * @var    boolean  True, to crop cover so that they are square.
	 * @since  1.0
	 */
	...
	protected $sizes = array(
				PROFILECOVER_SIZE_ORIGINAL,
				PROFILECOVER_SIZE_1000,
				PROFILECOVER_SIZE_200
				);

	/**
	 * @const  string	Profile Cover file extension
	 * @since  1.0
	 */
	...
	function onContentPrepareData($context, $data)
	{
		...
		if (is_object($data))
		{
			...
			// Load the profile cover from the database.
			$db = JFactory::getDbo();
			...
			$query->where('profile_key = '.$db->quote(plgUserProfileCover::PROFILE_KEY));
			$db->setQuery($query);
			...
			// Merge the coverprofile data.
			$data->profilecover['file'] = $result;
	...
	function onContentPrepareForm($form, $data)
	{
		...
		$lang->load('plg_user_profilecover', JPATH_ADMINISTRATOR);
		...
		$form->loadFile('profilecover', false);
	...
	function onUserAfterSave($data, $isNew, $result, $error)
	{
		...
		if( $files['error']['profilecover']['file'] == 0 && !empty($files['tmp_name']['profilecover']['file']) )
		{
			...
			if( 
				$sourceWidth < PROFILECOVER_SIZE_200
				||
				$sourceHeight < PROFILECOVER_SIZE_200
			) {
		...
		if( 
			...
			&&
			(
				!empty($post['profilecover']['file']['remove'])
				||
				$savedNewProfileCover
			)
		) 
		{
		...
		if( $userId && $savedNewProfileCover )
		{
			try
			{
				...
				$query->insert('#__user_profiles')
					...
					->values(
						$userId.', '
						.$db->quote(plgUserProfileCover::PROFILE_KEY).', '
	...
	function onUserAfterDelete($user, $success, $msg)
	{
		...
		return $this->removeProfileCover($userId);
	}
	
	/**
	 * Remove profile cover's file and table record
	 *
	 * @param	int	$userId		User ID
	 * 
	 * @return 	boolean	true if successfully removed profile cover.
	 * 			false if failed or there is no profile cover 
	 * 			to remove.
	 */
	function removeProfileCover($userId)
	{
		if ($userId)
		{
			try
			{
				...
				if( !empty($profile_value) )
				{
					...
					foreach( $this->sizes AS $size )
					{
						if($size == PROFILECOVER_SIZE_ORIGINAL)
						{
							JFile::delete(PROFILECOVER_PATH_ORIGINAL.$profile_value);
						} else {
							JFile::delete(constant('PROFILECOVER_PATH_'.$size).$profile_value);
						}
					}
					...
					$query->delete('#__user_profiles')
						...
						->where('profile_key = '.$db->quote(plgUserProfileCover::PROFILE_KEY));
					$db->setQuery($query);
