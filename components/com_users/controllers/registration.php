...
class UsersControllerRegistration extends UsersController
{
	...
	public function register()
	{
		...
		if (!$resp->is_valid) {
			echo '<script>alert("Mã xác nhận không đúng");window.history.go(-1);</script>';
		} else {
			...
			else
			{
				$this->setMessage(JText::_('COM_USERS_REGISTRATION_SAVE_SUCCESS'));
				//$this->setRedirect(JRoute::_('index.php?option=com_users&view=login', false));
				$this->setRedirect(JRoute::_('index.php?option=com_users&view=registration&layout=success', false));
			}
