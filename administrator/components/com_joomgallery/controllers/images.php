...
class JoomGalleryControllerImages extends JoomGalleryController
{
  ...
  public function cancel()
  {
	...
	public function special(){
		...
		$this->setSpecial($cid[0], 1);
		$this->setRedirect($this->_ambit->getRedirectUrl(), "Hình đã được chọn");
	}
	
	public function unspecial(){
		$cid = JRequest::getVar("cid");
		$this->setSpecial($cid[0], 0);
		$this->setRedirect($this->_ambit->getRedirectUrl(), "Hình đã được bỏ chọn");
	}
	
	public function setSpecial($id, $status){
		$db = JFactory::getDBO();
		$db->setQuery("UPDATE #__joomgallery SET special = $status WhERE id = $id");
		return $db->query();
	}
}